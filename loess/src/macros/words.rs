/// Defines keywords and identifiers.
///
/// # Examples
///
/// ```rust
	...
	(PeekFrom for $name:ty, $ident:pat => $condition:expr$(,)?) => {
		impl $crate::PeekFrom for $name {
			fn peek_from(input: &$crate::Input) -> bool {
				input.peek(|tts, _| matches!(tts, [$crate::__::TokenTree::Ident($ident)] if $condition))
			}
		}
	};
			...
			fn pop_parsed_from(
				...
				input
					.pop_or_replace(|tts, _| match tts {
						[$crate::__::TokenTree::Ident($ident)] if $condition => Ok(Self($ident)),
						tts => Err(tts),
					})
					.map_err(|spans| {
